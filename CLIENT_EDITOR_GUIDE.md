...
Open the [website editor](https://velocitymarketing.com.au/admin) and sign in with your email code. Mike can sign in with `mike@velocitymarketing.com.au`; Sam uses `sam@sampenny.io`. Both can edit, upload images, save, restore history and publish to the live website. Login codes are sent only when requested. The editor opens at its existing secure address and shows a **Live** badge. Refresh any older open editor tab before publishing; download unsaved edits first if needed.
...
5. Select **Save & publish live**. The button saves your changes and starts the live build. The progress panel shows the current step, elapsed time and an estimate based on recent successful updates (currently around 20–45 seconds). Keep the tab open until it says **Saved**, then you can keep editing or return later while the saved version builds. Wait for **Live site is up to date**, then open the live link to review. A slower build shows a delay message and keeps checking. The changes are then visible at velocitymarketing.com.au.
...
Saving also publishes live; there is no separate publishing step. If a build fails, your saved changes remain available. Use **Retry live update**. Mike and Sam can both publish to the live domain.
...
Select **Save & publish live** to save the email settings with the website version. History and restore include these settings. The live contact form uses these saved email settings after publication; the editor preview does not send email. Changing **Public contact details** still only changes the address displayed on the website. Contact Sam for editor access or help with a failed publication.
...
Choose **Home → Areas we service** to edit the homepage heading and introduction. The introduction explains the Australia and US reach and Mike’s connection to Gippsland from his Traralgon base. On the public homepage, the small heading expands the town list; the editor keeps it expanded for editing. Town names link directly to their pages beneath five region headings; regions do not have separate landing pages. Moe, Morwell and Traralgon appear in both Central Gippsland and Latrobe Valley and always link to the same town page.
...
Use **Save & publish live** for location edits, just as for Home. History includes every location’s content. Restoring a version saved before location pages were introduced preserves its existing copy and supplies the new pages’ starting content for review.
