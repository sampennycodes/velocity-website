...
The shared Contact form group stores two boolean flags for referral visibility and requiredness. Missing values in older snapshots default to visible and optional. The same packaged contact settings control server validation, and hidden referral answers are discarded. Other details are bounded to 500 characters and escaped in the internal notification. The CMS preview applies both flags immediately; contact delivery remains disabled on staging.
