...
function leadDetails({ name, email, phone }) {
  ...
  return [
    {
      ...
      html: emailLayout('New website enquiry', `${htmlText(intro)}${leadDetails(fields)}<h2 style="margin:0 0 12px;color:#746380;font-size:12px;line-height:1.6;letter-spacing:1px;text-transform:uppercase;">Message</h2>${htmlText(message)}`),
      text: `${intro}\n\nName: ${name}\nEmail: ${email}${phone ? `\nPhone: ${phone}` : ''}\n\nMessage:\n${message}`,
    },
    {
      from,
