import { z } from "zod";
import { groups, fields } from './location-fields.js';
export { groups, fields };
// Bounded full-site saves, including all 17 pages at their permitted field lengths.
export const contentRequestByteLimit = 4_000_000;
...
const schemas = Object.fromEntries(
  fields.map((f) => {
    ...
    if (f.type === "image") return [f.key, f.added ? image.default(f.initial) : image];
    ...
    if (f.added) schema = schema.default(f.initial);
    return [f.key, schema];
  }),
);
