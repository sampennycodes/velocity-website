import { z } from 'zod';
import { editorConfig, checkOrigin, publicationEnvironment, checkPublicationEnvironment } from './config.js';
...
import { contentSchema, contentRequestByteLimit } from '../content/model.js';
...
import { saveAndPublish, monitorPublication } from './save-publish.js';
import { loadWorkspace, loadStatus } from './workspace.js';
...
export async function handleEditor(request, env = process.env, dependencies = {}) {
  try {
    ...
    if (request.method === 'POST') {
      ...
      if (action === 'send-code') {
        ...
        return json({ message: 'If your account is enabled, a login code has been sent. Check your inbox.' });
      }
    ...
    const editor = await requireEditor(request, env, dependencies.auth);
    // Enforce the permission before parsing a write body or contacting storage.
    if (request.method === 'POST' || action === 'publish-status') requireContentWrite(editor);
    if (request.method === 'GET') {
      if (action === 'workspace') return json(await loadWorkspace(editor, env, dependencies.workspace));
      if (action === 'session') return json({ email: editor.email, role: editor.role, canSave: canSaveContent(editor), environment: publicationEnvironment(env) });
      ...
      if (action === 'status') return json(await loadStatus(env));
      ...
      if (action === 'save-publish') {
        ...
        const body = await readJson(request, z.object({ content: contentSchema, version: z.number().int().positive(), key: idSchema, environment: z.enum(['staging', 'production']).optional() }).strict(), contentRequestByteLimit);
        const result = await saveAndPublish(body, editor, env);
        if (result.publication && !['ready', 'failed'].includes(result.publication.status))
          dependencies.waitUntil?.(monitorPublication(result.publication.id, env));
        return json(result, 202);
      }
      if (action === 'save') {
        await rateLimit('draft-save', editor.email, 100, 3600, env);
        const body = await readJson(request, z.object({ content: contentSchema, version: z.number().int().positive() }).strict(), contentRequestByteLimit);
      ...
      if (action === 'publish') {
        await rateLimit('publish', editor.email, 10, 3600, env);
        const { key, version, environment } = await readJson(request, z.object({ key: idSchema, version: z.number().int().positive(), environment: z.enum(['staging', 'production']).optional() }).strict());
        checkPublicationEnvironment(environment, env);
        const job = await startContentPublication(key, version, editor, env);
        if (!['ready', 'failed'].includes(job.status)) dependencies.waitUntil?.(monitorPublication(job.id, env));
        return json(job, 202);
      }
    }
