...
export async function handleEditor(request, env = process.env, dependencies = {}) {
  try {
    ...
    if (request.method === 'GET') {
      if (action === 'session') return json({ email: editor.email, role: editor.role, canSave: canSaveContent(editor), environment: 'staging' });
      ...
      if (action === 'status') {
        ...
        const current = await getPool(env).query("SELECT EXISTS(SELECT 1 FROM velocity_editor.site_state s JOIN velocity_editor.content_publications p ON p.id=s.publication_id JOIN velocity_editor.drafts d ON d.id='site' WHERE s.environment='staging' AND p.status='ready' AND p.created_at >= d.updated_at) AS current");
        return json({ privateUploadsConfigured: Boolean(env.EDITOR_BLOB_PRIVATE_TOKEN), publishingConfigured: env.EDITOR_PUBLISH_ENABLED === 'true', stagingCurrent: current.rows[0].current, publications: rows });
      }
      ...
      if (action === 'save-publish') {
        ...
        const body = await readJson(request, z.object({ content: contentSchema, version: z.number().int().positive(), key: idSchema }).strict(), 256000);
