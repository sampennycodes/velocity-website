...
export function publicationProgress(job, estimate, now = Date.now()) {
  ...
  if (job?.status === 'ready') return { title: 'Staging is up to date', detail: `Updated in ${duration}. Your saved changes are visible on the staging site.`, elapsed: '', active: false };
  if (job?.status === 'failed') return { title: 'Saved · staging update failed', detail: 'Your draft is safe. Retry the staging update below.', elapsed: '', active: false };
  if (!job) return { title: 'Saving your changes', detail: `${usual} to save and update staging. Please keep this tab open until your draft is saved.`, elapsed: '', active: true };
  const title = job.phase === 'checking' ? 'Saved · checking the staging link' : job.phase === 'queued' ? 'Saved · waiting for the build to start' : job.status === 'unknown' ? 'Saved · waiting for build confirmation' : 'Saved · building the staging site';
  const detail = elapsed > estimate.maxSeconds
    ...
    : `${usual}. Your draft is saved. You can keep editing while staging updates.`;
