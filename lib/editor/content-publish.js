...
import { STAGING_ORIGIN, PRODUCTION_ORIGIN } from "../site.js";
...
export function contentDeploymentPayload(config, revisionId, jobId) {
  ...
  insist(['staging', 'production'].includes(environment), 400, 'Invalid publishing destination.');
  return {
    ...(environment === 'production' ? { target: 'production', build: { env: {
      VELOCITY_CONTENT_REVISION: revisionId, VELOCITY_CONTENT_JOB: jobId,
    } } } : {}),
    ...
    // Bind this deployment to an immutable publication. Never change the
    // project's shared build command or a shared revision pointer.
    meta: {
      velocityContentRevision: revisionId,
      velocityContentJob: jobId,
      velocitySourceSha: config.sha,
      velocityEnvironment: environment,
    },
  };
}
...
export function checkContentDeployment(deployment, job, config) {
  const environment = job.environment || 'staging';
  insist(['staging', 'production'].includes(environment), 502, 'Invalid publication destination.');
  insist(
    deployment.projectId === config.project &&
      deployment.ownerId === config.team &&
      (environment === 'production' ? deployment.target === 'production' : deployment.target == null),
    502,
    "The deployment does not match the publishing destination. Contact Sam.",
  );
  insist(
    deployment.meta?.velocityContentRevision === job.revision_id &&
      ...
      deployment.meta?.velocityEnvironment === environment,
    502,
...
export async function startContentPublication(key, version, editor, env) {
  ...
  insist(
    projectResponse.ok,
    503,
    "Publishing cannot access the website. Contact Sam.",
  );
  ...
  try {
    ...
    if (duplicate.rows[0]) {
      insist(duplicate.rows[0].environment === config.environment, 409, 'This request belongs to a different publishing destination. Refresh the editor.');
      await client.query("COMMIT");
      return result(duplicate.rows[0]);
    }
    const active = await client.query(
      "SELECT id FROM velocity_editor.content_publications WHERE environment=$1 AND status IN ('preparing','building','unknown')",
      [config.environment],
    );
    insist(
      !active.rowCount,
    ...
    const inserted = await client.query(
      "INSERT INTO velocity_editor.content_publications (id,revision_id,source_sha,idempotency_key,environment) VALUES ($1,$2,$3,$4,$5) RETURNING *",
      [randomUUID(), draft.rows[0].revision_id, config.sha, key, config.environment],
    );
    job = inserted.rows[0];
    await client.query("COMMIT");
  } catch (e) {
...
export async function contentPublicationStatus(id, env) {
  ...
  const publicOrigin = job.environment === 'production' ? PRODUCTION_ORIGIN : STAGING_ORIGIN;
  if (deployment.readyState === 'READY') {
    // READY alone is insufficient: the link Mike uses must serve this revision.
    const visible = await fetch(`${publicOrigin}/?editor-revision=${job.revision_id}`, {
    ...
    if (!tag?.includes(job.revision_id))
      return { ...result(job), status: 'building', phase: 'checking', error: 'Build ready. Waiting for the public link to serve the saved changes.' };
  }
  const status =
    deployment.readyState === "READY"
      ...
      : ["ERROR", "CANCELED"].includes(deployment.readyState)
        ? "failed"
        : "building";
  const url = status === 'ready' ? publicOrigin : null;
  const error =
    status === "failed"
  ...
  try {
    ...
    if (status === "ready")
      await client.query(
        "INSERT INTO velocity_editor.site_state (environment,revision_id,publication_id) VALUES ($4,$1,$2) ON CONFLICT(environment) DO UPDATE SET revision_id=$1,publication_id=$2,updated_at=now() WHERE (SELECT created_at FROM velocity_editor.content_publications WHERE id=velocity_editor.site_state.publication_id) <= $3",
        [job.revision_id, id, job.created_at, job.environment],
      );
    await client.query("COMMIT");
