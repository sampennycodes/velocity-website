...
export async function contentPublicationStatus(id, env) {
  ...
  const stagingOrigin = 'https://velocity-website-git-codex-visual-refresh-sam-pennys-projects.vercel.app';
  if (deployment.readyState === 'READY') {
    ...
    if (!tag?.includes(job.revision_id))
      return { ...result(job), status: 'building', error: 'Build ready. Waiting for the staging link to serve the saved changes.' };
  }
  const status =
  ...
  return { ...result(job), status, url, error };
}
