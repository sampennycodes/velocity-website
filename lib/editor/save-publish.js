...
import { publishConfig, checkPublicationEnvironment } from './config.js';
...
export async function saveAndPublish(body, editor, env, dependencies = {}) {
  ...
  checkPublicationEnvironment(body.environment, env);
  publishConfig(env);
  ...
  try {
    ...
    return { draft, publication: null, publishError: error instanceof EditorError ? error.message : 'Your draft was saved, but the website could not be updated. Please retry the update.' };
  }
}
