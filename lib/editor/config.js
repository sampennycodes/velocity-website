...
export function editorConfig(env) {
  ...
  // Phase 1 deliberately has no production publishing path.
...
export function publishConfig(env) {
  insist(env.EDITOR_PUBLISH_ENABLED === 'true', 503, 'Staging publishing is awaiting configuration.');
  insist(env.EDITOR_ENVIRONMENT === 'staging' && env.VERCEL_ENV !== 'production', 403, 'Only staging publishing is available.');
  ...
  return { team: env.EDITOR_VERCEL_TEAM_ID, project: env.EDITOR_VERCEL_PROJECT_ID, token: env.EDITOR_VERCEL_TOKEN, sha: env.EDITOR_APPROVED_SOURCE_SHA };
}
